...
string construct_candidates(Game &game,
    ...
    for (char c : candidates) {
        ...
        game.move(c);
        auto hash_bitset = game.key();

        if (visited_game_states.find(hash_bitset) == visited_game_states.end()) {
            int distance = game.get_manhattan_distance();
            if (distance + game.get_moves_so_far().size() < 150) {
                new_candidates.push_back({c, distance});
                visited_game_states.insert(hash_bitset);
            }
        }
