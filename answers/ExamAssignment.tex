...
Since the tasks asks for to search for a sub-graph in which all nodes are connected, but all unnecessary edges have been removed, following that the edge weights are minimal, one needs
to find a minimal spanning tree. In class, we discussed only Prims algorithm and Kruskal's algorithm. Since no runtime limit was given and Prims algorithm was easier to implement, I chose
to go with Prims algorithm.
