...
inline uint32_t reverse_with_lookup(uint32_t num) {
    ...
    for (uint8_t i = 0; i < 32; i += 8) {
        // get reversed for each byte in the corresponding iteration
        uint32_t mask = 255 << i;  // one byte mask (shifted by byte)
        uint8_t byte = (num & mask) >> i;
        byte = lookup[byte];   // get look up byte
        result = result << 8;  // make space for new byte
        result |= byte;        // put the looked up byte into the first 8 bits
    }
