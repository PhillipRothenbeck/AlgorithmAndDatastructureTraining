...
inline void clean_spaces(string &s) {
  ...
  char *occ = strstr(s.data(), "  ");
  size_t length = s.length() - ((char *) &s - occ);
  std::cout << length << std::endl;
  size_t count_deleted_spaces = 0;
  while(occ != nullptr) {
    size_t occ_count = 1;
    while(occ[occ_count] == ' ') {
      occ_count++;
    ...
    occ_count;
    std::cout << occ[0] << " " << occ[occ_count] << std::endl;
    count_deleted_spaces += occ_count - 2;
    memmove(occ + 1, occ + occ_count, length);
    std::cout << occ[0] << " " << occ[occ_count] << std::endl;

    char* tmp_occ;
    occ = strstr(tmp_occ, "  ");
    length = length - (occ - tmp_occ);
    occ = tmp_occ;
