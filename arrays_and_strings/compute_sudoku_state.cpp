...
state compute_sudoku_state(const string &board) {
  std::cout << "Begin analyzing new Sodoku" << std::endl;
  ...
  line_set.reset();
  state sodoku_state = state::solved;
  size_t idx = 0;
  // check line correctness
  while(idx < 81) {
    if(board[idx] == ' ') {
      //if a ' ' appears the sodoku can't be valid anymore
      sodoku_state = state::valid;
    } else {
  ...
  idx = 0;
  line_set.reset();
  // travers the board column for column
  ...
  idx = 0;
  line_set.reset();
  //travers board square for sqare
